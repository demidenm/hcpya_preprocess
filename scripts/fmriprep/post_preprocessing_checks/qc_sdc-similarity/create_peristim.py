...
# paths,files,sort
inp_eventfiles = glob(f'{input_dir}/sub-{sub}/ses-{ses}/func/{sub}_ses-{ses}_task-{task}_dir-*_run-*_events.tsv')
inp_boldfiles = glob(f'{input_dir}/sub-{sub}/ses-{ses}/func/{sub}_ses-{ses}_task-{task}_dir-*_run-*_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz')
...
# lock to events
out_df = masked_timeseries.extract_postcue_trs_for_conditions(events_data=inp_eventfiles, onset=onsetcol, 
...
masked_timeseries.plot_responses(df=out_df, tr=scan_tr, delay=tr_delay, style= 'white', save_path = f'{savedir}/{sub}_ses-{ses}_task-{task}_plot-peristim.tsv', show_plot=False, ylim = (-2, 2))
...
max_mean_signal_row = summary_df[0:14].loc[summary_df['Mean_Signal'].idxmax()]
